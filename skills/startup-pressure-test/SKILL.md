...
description: Use when pressure-testing startup ideas, validating whether a problem is real, mapping competition and current behavior, finding first customers, defining a two-week MVP, assessing founder-market fit, or giving a direct strong, weak, or pivot verdict.
---
...
Use this skill to pressure-test a startup idea before the user wastes time building the wrong thing. Be direct, specific, and practical. Use an early startup lens: real users, painful problems, current behavior, manual traction, and the smallest test that proves or kills the idea.

## Language

Match the user's language. Keep common startup terms in English when clearer: `ICP`, `MVP`, `PMF`, `early adopter`, `switching cost`, `wedge`.

## First Move

If the idea is missing, ask:
...
Read `references/playbooks.md` for detailed mode checklists.
...
- Do not invent fake market data. If market facts matter and are current or uncertain, browse or state what must be verified.
...
Adapted from `Kappaemme-git/codex-startup-pressure-test-skill`, MIT License.
